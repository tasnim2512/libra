...
/**
 * Base application context (generic to work with any bindings/variables)
 */
export type BaseContext<
  TBindings extends Record<string, unknown> = BaseBindings,
  TVariables extends Record<string, unknown> = BaseVariables
> = Context<{ Bindings: TBindings; Variables: TVariables }>

/**
 * Default base context for backward compatibility
 */
export type DefaultBaseContext = BaseContext<BaseBindings, BaseVariables>
 ...
 */
export interface RateLimitConfig<TContext = DefaultBaseContext> {
  windowMs: number
  maxRequests: number
  keyGenerator?: (c: TContext) => string
 ...
 */
export interface CloudflareRateLimitConfig<TContext = DefaultBaseContext> {
  keyGenerator?: (c: TContext) => string
  message?: string
...
/**
 * Error handler function type (generic to work with any context)
 */
export type ErrorHandlerFunction<TContext = DefaultBaseContext> = (err: Error, c: TContext) => Response | Promise<Response>
